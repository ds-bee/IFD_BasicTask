...
# from datasets_process.MatrixDatasets import dataset
# from datasets_process.matrix_aug import *
import pickle
import pywt
...
def get_files(root, test=False):
    ...
    '''
    path = os.path.join('/data',root,dataname[1])
    data = loadmat(path)
...
class UoCCWT(object):
    ...
    def data_preprare(self, test=False):
        if len(os.path.basename(self.data_dir).split('.')) == 2:
            ...
            with open(os.path.join(self.data_dir, "UoCCWT.pkl"), 'wb') as fo:
